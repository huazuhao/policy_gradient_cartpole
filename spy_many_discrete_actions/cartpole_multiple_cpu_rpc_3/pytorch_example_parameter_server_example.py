...
batch_size = 20 #for each iteration of update and each trainer, we have 20 samples
image_w = 64
image_h = 64
num_classes = 30
batch_update_size = 2 #how many trainers we have
num_batches = 6 #how many iterations of parameter update (this is like epoches)
...
class Trainer(object):
    ...
    def train(self):
        timed_log("trainer.train is being called")
        name = rpc.get_worker_info().name
        m = self.ps_rref.rpc_sync().get_model().cuda()
        timed_log("before for loop in trainer.train")
        for inputs, labels in self.get_next_batch():
            timed_log("after for loop in trainer.train")
