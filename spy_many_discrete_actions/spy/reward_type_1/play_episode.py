...
def play_episode(environment, device, action_space_size, agent, gamma, episode):
        ...
        for step_index in range(0,C.max_simulation_length):
            ...
            action_logits = agent(torch.tensor(nn_input).float().unsqueeze(dim=0).to(device))
            ...
            state, reward, execute_action = environment.step(action=action.cpu().item())
