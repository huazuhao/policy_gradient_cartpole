...
if __name__ == '__main__':
    ...
    theta, episode_rewards = train(N=50, T=300, delta=1e-3)

    #store the best theta
    best_theta = {}
    best_theta['theta'] = theta

    cwd = os.getcwd()
    #cwd = os.path.join(cwd, 'data_folder')
    parameter_file = 'best_theta.json'
    cwd = os.path.join(cwd,parameter_file)
    with open(cwd, 'w') as statusFile:
        statusFile.write(jsonpickle.encode(best_theta))

    #store the optimization history
    optimization_history = {}
    optimization_history['episode_rewards'] = episode_rewards

    cwd = os.getcwd()
    #cwd = os.path.join(cwd, 'data_folder')
    parameter_file = 'policy_gradient_optimization_history.json'
    cwd = os.path.join(cwd,parameter_file)
    with open(cwd, 'w') as statusFile:
