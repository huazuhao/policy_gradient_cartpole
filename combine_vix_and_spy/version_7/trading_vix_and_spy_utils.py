...
def compute_volatility_feature(input_dataframe,input_column_name,data_window):

    slope_data = []

    for end_index in range(input_dataframe.shape[0],data_window-1,-1):
        y = input_dataframe[input_column_name].iloc[end_index-data_window:end_index].tolist()
        slope_data.append(np.std(y))

    #add some zeros before
    for _ in range(0,data_window-1):
        slope_data.append(0)
        
    return slope_data[::-1]

def compute_previous_slope_feature(input_dataframe,input_column_name,data_window,previous_multiplier):

    linear_regression_length = data_window
    most_recent_multiplier = previous_multiplier #this could 2, 3...
    ...
    for end_index in range(input_dataframe.shape[0],linear_regression_length*(most_recent_multiplier)-1,-1):
        y = input_dataframe[input_column_name].iloc[end_index-linear_regression_length:end_index].tolist()
        y = np.asarray(y)
        y = np.reshape(y,(-1,1))
    ...
    #add some zeros before
    for _ in range(0,linear_regression_length*(most_recent_multiplier)-1):
        slope_data.append(0)

    return slope_data[::-1]

def compute_previous_volatility_feature(input_dataframe,input_column_name,data_window,previous_multiplier):

    most_recent_multiplier = previous_multiplier #this could 2, 3...

    slope_data = []

    for end_index in range(input_dataframe.shape[0],data_window*(most_recent_multiplier)-1,-1):
        y = input_dataframe[input_column_name].iloc[end_index-data_window:end_index].tolist()

        slope_data.append(np.std(y))

    #add some zeros before
    for _ in range(0,data_window*(most_recent_multiplier)-1):
        slope_data.append(0)
...
def compute_total_portfolio_value_with_vix_and_spy(vix_cash,vix_amount,vix_price,spy_cash,spy_positions,spy_price):
    total_value = 0
...
def total_value_in_spy_stock(spy_cash,spy_positions,spy_price):
    value_in_spy = 0
    for position in spy_positions:
        value_in_spy += position['quantity']*spy_price
...
def compute_single_commission(stock_count):
    return C.commission_per_stock*stock_count