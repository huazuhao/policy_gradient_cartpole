...
class optimization():
    ...
    def evaluate_parameter(self,parameter, weight=None):
        ...
        if np.mean(total_returns) - np.std(total_returns) * 0.5>self.best_obj_so_far:
            self.best_obj_so_far = np.mean(total_returns) - np.std(total_returns) * 0.5
        ...
        print('the objective for ths iteration is',np.mean(total_returns) - np.std(total_returns) * 0.5)
        print('the mean return is',np.mean(total_returns))
        print('the current time is',datetime.datetime.now())
        print(' ')
    ...
    def get_one_sample_score(self,q,free_parameters):
        ...
        #the features are
        #1. current vix index adj close level
        #2. vix index mv ratio
        #3. days since vix index spike over 5
        #4. days since vix index spike over 6
        #5. days since vix index spike over 7
        #6. current vix/cash ratio in the portfolio
    ...
    def init_search_space(self):
        ...
        parameters=[
            ...
            RangeParameter(name="cov_para_11", parameter_type=ParameterType.FLOAT, lower=-0.9, upper=0.9),
            RangeParameter(name="cov_para_12", parameter_type=ParameterType.FLOAT, lower=-0.9, upper=0.9),
            RangeParameter(name="cov_para_13", parameter_type=ParameterType.FLOAT, lower=-0.9, upper=0.9),
            RangeParameter(name="cov_para_14", parameter_type=ParameterType.FLOAT, lower=-0.9, upper=0.9),
            RangeParameter(name="cov_para_15", parameter_type=ParameterType.FLOAT, lower=-0.9, upper=0.9),
            ...
            RangeParameter(name="beta_6a", parameter_type=ParameterType.FLOAT, lower=0, upper=20),
            RangeParameter(name="beta_6b", parameter_type=ParameterType.FLOAT, lower=0, upper=20),

            RangeParameter(name="lambda_expon_0", parameter_type=ParameterType.FLOAT, lower=0.01, upper=0.1),
            RangeParameter(name="lambda_expon_1", parameter_type=ParameterType.FLOAT, lower=0.01, upper=1),
    ...
    def optimization_trials(self):
        ...
        for i in range(C.OPTIMIZATION_TRIALS):
            print('Running GP+EI optimization trial', i)
