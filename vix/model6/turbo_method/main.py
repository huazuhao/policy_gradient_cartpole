...
import os
import optimization_trading_vix_turbo
import numpy as np
import matplotlib
import jsonpickle
...
if __name__ == '__main__':
    ...
    f = optimization_trading_vix_turbo.optimization_trading_vix_turbo()
    ...
    turbo_m = TurboM(
        ...
        n_init=40,  # Number of initial trials from an Symmetric Latin hypercube design
        max_evals=3000,  # Maximum number of evaluations
    ...
    parameter_file = 'best_free_parameters_from_turbo.json'
    ...
    cwd = os.getcwd()
    #cwd = os.path.join(cwd, 'data_folder')
    parameter_file = 'optimization_process_data.json'
    cwd = os.path.join(cwd,parameter_file)
    with open(cwd, 'w') as statusFile:
