...
def ppo_learn(replay_buffer,replay_buffer_reward,env,model,cov_matrix,model_optim):
    ...
    while True:
        ...
        if np.mean(new_sample_reward) > current_best_reward:
            ...
            cwd = os.getcwd()
            parameter_file = 'bipedal_nn_trained_model.pt'
            cwd = os.path.join(cwd,parameter_file)
        ...
        for update_iteration in range(0,5):
            ...
            for entry in replay_buffer_reward:
                new_replay_buffer_reward.append(np.log(entry+1000)) #make sure all the rewards are positive
            sample_probability = np.exp(new_replay_buffer_reward) / np.sum(np.exp(new_replay_buffer_reward)) #softmax 
