...
def sample_one_trajectory(theta,env):
    ...
    for time_index in range(0,200):
        ...
        action, log_prob = utils.compute_action_distribution(theta, current_feature, mode = 'train')
        ...
        this_trajectory_actions.append(action)
...
def sample(theta, env, N, replay_buffer, replay_buffer_rewards):
    ...
    for _ in range(0,N):
        one_trajectory_data,one_trajectory_total_reward = sample_one_trajectory(theta,env)
...
def train(N, T, delta,env):
    ...
    for iteration_index in range(0,T):
        ...
        replay_buffer,replay_buffer_rewards,current_batch_reward = sample(theta, env, N, replay_buffer, replay_buffer_rewards)
        ...
        for _ in range(0,C.max_offline_training):
            ...
            value_function_gradient = utils.compute_value_gradient(sampled_off_line_data)
